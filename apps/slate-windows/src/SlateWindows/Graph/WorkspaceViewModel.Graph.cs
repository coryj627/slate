...
internal sealed partial class WorkspaceViewModel
{
    ...
    private readonly GraphNavigator _graphNavigator;

    internal GraphNavigator GraphNavigator => _graphNavigator;

    /// <summary>The open's admission (rule P, Term P3): null admits.
    /// Windows has no navigation gate today (C-D1); the facts inject a
    /// refusal through it.</summary>
    public Func<string?>? GraphOpenAdmissionReason { get; set; }

    /// <summary>The preset ARM (Term P2): a cause-shaped field, set by the
    /// funnel before the open's mutation, consumed by the follow method at
    /// the one transition it loads for, cleared by the outermost boundary
    /// when unconsumed. Not a cause: AD-12's two causes stand.</summary>
    private GraphPreset? _graphPresetArm;
    private bool _graphArmConsumed;

    internal GraphPreset? GraphPresetArmForTests => _graphPresetArm;
    ...
    /// cause (CD-5; Term P2) — and the boundary's verdict returned: whether
    /// the follow method consumed the arm, and whether the graph is
    /// effective now.</summary>
    private GraphPresetOpenReport OpenGraphForPreset(GraphPreset preset)
    {
        _graphPresetArm = preset;
        _graphArmConsumed = false;
        RunWorkspaceMutation(() => OpenItem(
            new WorkspaceItemState(WorkspaceItemKind.Graph, "graph:singleton"),
            WorkspaceOpenTarget.NewTab));
        return new GraphPresetOpenReport(_graphArmConsumed, GraphTabIsEffective());
    }

    private RelayCommand? _graphOrphansCommand;
    private RelayCommand? _graphUnresolvedCommand;
    private RelayCommand? _graphMostLinkedCommand;

    /// <summary>`slate.graph.orphans` (C-3): the navigator's preset verb;
    /// always enabled — the admission is the verb's own (Term P3).</summary>
    public System.Windows.Input.ICommand GraphOrphansCommand =>
        _graphOrphansCommand ??= new RelayCommand(_ => _graphNavigator.RunPreset(GraphPreset.Orphans), _ => true);

    /// <summary>`slate.graph.unresolved` (C-3).</summary>
    public System.Windows.Input.ICommand GraphUnresolvedCommand =>
        _graphUnresolvedCommand ??= new RelayCommand(_ => _graphNavigator.RunPreset(GraphPreset.Unresolved), _ => true);

    /// <summary>`slate.graph.mostLinked` (C-3).</summary>
    ...
    private void GraphFollowActiveTab()
    {
        ...
        {
            GraphLoadsForTests++;
            // Rule P, Term P1 (a): the arm COLOURS the query-change load at
            // the one transition Term 4 loads for — policy Preset, the
            // preset on the token, the default sort — and is consumed; a
            // transition that does not load consumes nothing.
            GraphPreset? arm = _graphPresetArm;
            if (arm is not null)
            {
                _graphPresetArm = null;
                _graphArmConsumed = true;
            }
            _ = _graphDocument.Load(
                GraphLoadKind.Pair,
                arm is null ? GraphAnnouncePolicy.Summary : GraphAnnouncePolicy.Preset,
                preset: arm);
        }
    }
    ...
    /// graph's transition did not consume is cleared here — and the preset
    /// ARM with it (Term P2), its consumption already reported to the
    /// funnel that set it.</summary>
    private void ClearGraphCauseAtMutationBoundary()
    {
        _graphCause = GraphActivationCause.Activation;
        _graphPresetArm = null;
    }
