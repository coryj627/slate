...
//!   the file is deleted, YANA rebuilds from the vault on next open. See
...
//!   migrations (i.e. a database from a newer YANA version), per the
...
pub enum DbError {
    ...
    #[error(
        "database is at schema version {db_version}, newer than this build's max version {runner_max}; \
         upgrade YANA or restore the vault from a backup"
    )]
