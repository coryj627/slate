...
mod surface {
    ...
    /// W6-2 PR 0a (contracts doc 0a-7): every count the two summaries
    /// speak is exported on the record and asserted on its own, so the
    /// typed summary events cannot agree with `audio_summary` while both
    /// are wrong. The vault: `a` links `b` twice and embeds it once
    /// (one Link edge of count 2, one Embed edge of count 1), links a
    /// ghost, and `b` links back; `far` links `a` (a hop beyond depth 1
    /// from `b`); `pic.png` is embedded by `a` (an attachment, filtered
    /// out by default and reachable as a centre only when shown).
    #[test]
    fn summary_counts_are_the_index_counts() {
        let (_tmp, session) = make_vault(|p| {
            p.write_file("a.md", b"[[b]] [[b]] ![[b]] [[Missing Note]] ![[pic.png]]")
                .unwrap();
            p.write_file("b.md", b"[[a]]").unwrap();
            p.write_file("far.md", b"[[a]]").unwrap();
            p.write_file("loner.md", b"alone").unwrap();
            p.write_file("pic.png", b"x").unwrap();
        });
        session.scan_initial(&CancelToken::new()).unwrap();

        // Default filter: attachments off (pic.png and its embed drop),
        // ghosts on. Edges: a->b Link(2), a->b Embed(1), a->ghost
        // Link(1), b->a Link(1), far->a Link(1) = 6 references, Link AND
        // Embed alike.
        let snap = session.graph_snapshot(GraphFilter::default()).unwrap();
        let c = &snap.summary_counts;
        assert_eq!(c.notes, 4, "a, b, far, loner");
        assert_eq!(c.links, 6, "the sum of every edge's count, embeds included");
        assert_eq!(c.orphans, 1, "loner");
        assert_eq!(c.unresolved, 1, "Missing Note");
        assert!(!c.filtered, "the default filter is not a deviation");
        assert_eq!(
            snap.audio_summary,
            crate::graph_summary::snapshot_summary(c),
            "audio_summary is the formatter over the exported counts"
        );
        ...
        let shown = session
            ...
            .unwrap();
        let c = &shown.summary_counts;
        assert_eq!(c.notes, 4, "an attachment is not a note");
        assert_eq!(c.links, 7, "the pic.png embed now counts");
        assert!(c.filtered);

        // Neighbourhood of `b` at depth 1: members a, b (far is two hops
        // away). The centre's degrees are its FULL-GRAPH Link-only
        // metrics: in 2 (a's two links; a's embed is not a link), out 1
        // (b -> a) — unaffected by the depth that excludes `far`.
        let hood = session
            .graph_neighborhood("b.md", 1, GraphFilter::default())
            .unwrap();
        let c = &hood.summary_counts;
        assert_eq!(c.center_label, "b");
        assert_eq!(c.in_links, 2, "Link edges only, embeds excluded");
        assert_eq!(c.out_links, 1);
        assert_eq!(c.note_count, 2, "a and the centre itself");
        assert_eq!(c.depth, 1);
        assert_eq!(hood.nodes.len(), 2, "far is beyond depth 1");
        assert_eq!(
            hood.audio_summary,
            crate::graph_summary::neighborhood_summary(c)
        );

        // Depth 2 reaches `far`; the centre's degrees do not move.
        let deeper = session
            .graph_neighborhood("b.md", 2, GraphFilter::default())
            .unwrap();
        assert_eq!(deeper.summary_counts.in_links, 2);
        assert_eq!(deeper.summary_counts.out_links, 1);
        assert_eq!(deeper.summary_counts.note_count, 3, "a, b, far");
        assert_eq!(deeper.summary_counts.depth, 2);

        // An attachment centre counts no note for itself; its degrees are
        // zero (embeds are not links).
        let pic = session
            .graph_neighborhood(
                "pic.png",
                1,
            ...
            .unwrap();
        let c = &pic.summary_counts;
        assert_eq!(c.center_label, "pic.png");
        assert_eq!((c.in_links, c.out_links), (0, 0));
        assert_eq!(
            c.note_count, 1,
            "a, the embedding note; the attachment centre is not a note"
        );
    }

    /// The formatter-consistency half (0a-7): the typed events rendered
    /// from the exported counts equal the records' `audio_summary` byte
    /// for byte — which cannot stand alone, hence the test above.
    #[test]
    fn summary_events_render_the_snapshot_fields_verbatim() {
        let (_tmp, session) = fixture();
        let snap = session.graph_snapshot(GraphFilter::default()).unwrap();
        let event = crate::a11y::A11yEvent::Graph {
            event: crate::a11y::GraphA11yEvent::GraphSnapshotSummary {
                counts: snap.summary_counts,
            },
        };
        assert_eq!(event.render(), snap.audio_summary);
        assert_eq!(
            event.render(),
            "3 notes, 5 links. 1 orphans, 1 unresolved targets."
        );
        ...
        let hood = session
            ...
            .unwrap();
        let event = crate::a11y::A11yEvent::Graph {
            event: crate::a11y::GraphA11yEvent::GraphNeighborhoodSummary {
                counts: hood.summary_counts.clone(),
            },
        };
        assert_eq!(event.render(), hood.audio_summary);
        assert_eq!(
            event.render(),
