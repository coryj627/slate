...
/// matrix of contracts doc 0a-17, seventy-one entries — one per variant,
...
fn graph_corpus() -> Vec<GraphA11yEvent> {
    ...
    vec![
        GraphRow {
            verbosity: Standard,
            row: row("Alpha", Note, 3, 1, 0, false),
        },
        GraphRow {
            verbosity: Standard,
            row: row("Missing Note", Ghost, 0, 0, 2, false),
        },
        GraphRow {
            verbosity: Standard,
            row: row("Pic", Note, 1, 0, 0, true),
        },
        GraphRow {
            verbosity: Terse,
            row: row("Alpha", Note, 3, 1, 0, false),
        ...
        GraphRow {
            verbosity: Verbose,
            row: row("Hub", Note, 1024, 0, 0, false),
        },
        GraphRow {
            verbosity: Standard,
            row: row("diagram.png", Attachment, 0, 2, 0, false),
        },
        GraphRow {
            verbosity: Standard,
        ...
        GraphRow {
            verbosity: Standard,
            row: row("Todo", Ghost, 0, 0, 1, false),
        },
        GraphRow {
            verbosity: Standard,
        ...
        GraphWhereAmI {
            selection: GraphWhereAmISelection::Node {
                row: row("diagram.png", Attachment, 1, 2, 0, false),
                component: 7,
            },
            zoom_percent: 50,
            filter: GraphWhereAmIFilter::Normal {
...
mod tests {
    ...
    fn corpus_renders_the_shipped_strings() {
        let expected: Vec<(A11yPriority, &str)> = vec![
            ...
            (Medium, "Alpha"),
            (Medium, "Hub, 1024 links in, 0 links out"),
            (Medium, "diagram.png, 0 links in, 2 links out"),
            ...
            (
                Medium,
                "diagram.png, 1 links in, 2 links out, component 7, zoom 50 percent, filters: attachments shown, unresolved hidden.",
            ),
            (
                Medium,
    ...
    fn graph_where_am_i_renders_every_state_exactly() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "diagram.png, 1 links in, 2 links out, component 7, zoom 50 percent, filters: attachments shown, unresolved hidden.".to_owned(),
    ...
    fn graph_witnesses_obey_the_payload_invariants() {
        ...
        for event in graph_corpus() {
            match &event {
                ...
                GraphZoom { percent, .. } => {
                    assert!(*percent >= 1, "a zoom percent: {event:?}");
                }
                ...
                GraphNeighborsContent { labels } => {
                    ...
                    assert_eq!(
                        unique.len(),
                        labels.len(),
                        "neighbour labels are unique: {event:?}"
                    );
                }
                GraphWhereAmI {
                    selection,
                        ...
                        if let Some(needle) = name_filter {
                            ...
                            if !needle.is_empty() {
                                assert!(
                                    ...
                                    "under a name filter only matching labels exist: {event:?}"
                                );
                            }
                        }
                    }
    ...
    fn graph_where_am_i_witnesses_are_host_reachable() {
        ...
        assert_eq!(seen, 7, "the matrix carries seven Where-am-I witnesses");
    }
    ...
    fn the_graph_family_witness_count_is_pinned() {
        assert_eq!(
            graph_corpus().len(),
            71,
            "the graph witness matrix has 71 ordered entries"
        );
    }
    ...
    fn the_graph_witnesses_are_exactly_the_matrix_in_order() {
        ...
        let expected: Vec<&str> = vec![
            "GraphRow { verbosity: Standard, row: GraphRowCopy { label: \"Alpha\", kind: Note, in_links: 3, out_links: 1, references: 0, embed: false } }",
            "GraphRow { verbosity: Standard, row: GraphRowCopy { label: \"Missing Note\", kind: Ghost, in_links: 0, out_links: 0, references: 2, embed: false } }",
            "GraphRow { verbosity: Standard, row: GraphRowCopy { label: \"Pic\", kind: Note, in_links: 1, out_links: 0, references: 0, embed: true } }",
            "GraphRow { verbosity: Terse, row: GraphRowCopy { label: \"Alpha\", kind: Note, in_links: 3, out_links: 1, references: 0, embed: false } }",
            "GraphRow { verbosity: Verbose, row: GraphRowCopy { label: \"Hub\", kind: Note, in_links: 1024, out_links: 0, references: 0, embed: false } }",
            "GraphRow { verbosity: Standard, row: GraphRowCopy { label: \"diagram.png\", kind: Attachment, in_links: 0, out_links: 2, references: 0, embed: false } }",
            "GraphRow { verbosity: Standard, row: GraphRowCopy { label: \"Draft\", kind: Ghost, in_links: 0, out_links: 0, references: 0, embed: false } }",
            "GraphRow { verbosity: Standard, row: GraphRowCopy { label: \"Todo\", kind: Ghost, in_links: 0, out_links: 0, references: 1, embed: false } }",
            ...
            "GraphWhereAmI { selection: Node { row: GraphRowCopy { label: \"Missing Note\", kind: Ghost, in_links: 0, out_links: 0, references: 2, embed: false }, component: 0 }, zoom_percent: 250, filter: UnresolvedOnly, name_filter: None }",
            ...
            "GraphWhereAmI { selection: Node { row: GraphRowCopy { label: \"diagram.png\", kind: Attachment, in_links: 1, out_links: 2, references: 0, embed: false }, component: 7 }, zoom_percent: 50, filter: Normal { orphans_only: false, attachments_shown: true, ghosts_shown: false }, name_filter: None }",
            "GraphWhereAmI { selection: Node { row: GraphRowCopy { label: \"Alpha\", kind: Note, in_links: 3, out_links: 1, references: 0, embed: false }, component: 2 }, zoom_percent: 100, filter: Normal { orphans_only: false, attachments_shown: false, ghosts_shown: true }, name_filter: Some(\"\\u{2003}alpha\\u{a0}\") }",
            ...
            "GraphWhereAmI { selection: Node { row: GraphRowCopy { label: \"Todo\", kind: Ghost, in_links: 0, out_links: 0, references: 1, embed: false }, component: 3 }, zoom_percent: 80, filter: UnresolvedOnly, name_filter: None }",
            "GraphTierEntered",
            "GraphTierSummary { count: 2000 }",
            "GraphTierSummary { count: 1501 }",
