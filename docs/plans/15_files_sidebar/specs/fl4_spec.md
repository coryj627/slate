...
6. `audio_summary` (normative): `"{n} results."`; scoped: `"{n} results in {folder}."`; tag-scoped: `"{n} results for #{tag}."` — grouped decimals; `n = 1` takes the singular noun in every shape (`"1 result."`, `"1 result in {folder}."`, `"1 result for #{tag}."`); 0 ⇒ `"No results."`, and a scoped 0 still names its scope: `"No results in {folder}."` / `"No results for #{tag}."` (W7-7 #1250 amendment: a tag scope never shows in the filter field, so the summary is where the reader learns which tag is filtering). A clear (Windows' Clear Sidebar Filter, or the field emptied by the user) drops the text and any tag scope in one change and is followed by no filter run, so on Windows the always-visible sidebar status line takes core's `SidebarFilterCleared` sentence ("Filter cleared.") in place of the cleared filter's summary and that event is spoken once; mac's filter overlay, and the summary it carries, disappears on Escape, which stays silent (a proposed Mac designation in `a11y_trigger_designations.json`). Only an empty field is a clear: a whitespace-only edit keeps a tag scope, and core runs the trimmed query inside it.
