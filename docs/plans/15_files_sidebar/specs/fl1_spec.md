...
2. **Date line** (the existing secondary line) becomes configurable: source `modified | created` (default modified), format `relative | absolute` (default relative; absolute uses the user locale). For created display, a non-NULL `created_date` takes precedence over `created_ms`. Resolve its ISO numeric components once with `Calendar(identifier: .gregorian)` configured to the user's current time zone, strict Gregorian round-trip validation, and local-start construction; never give those components to `Calendar.current`. Presentation may then localize language, order, and calendar rendering from the resulting absolute `Date`, and relative display compares that same value. Buddhist, Hebrew, Islamic, or other system-calendar preferences may affect formatting but must not reinterpret the authored Gregorian year/month/day. A datetime or birthtime uses `created_ms` as an instant. Only when both fields are NULL does `created` fall back to modified **with the label "Modified"** — never mislabel a date.
...
Tests (`FileTreeSidebarTests.swift` pattern): titled vs untitled fixture rows; canonical date-only resolves to the authored Gregorian civil day's local start in timezones on both sides of UTC and across DST boundaries; injected Buddhist and Hebrew (or Islamic) current calendars produce that identical absolute value instead of interpreting `YYYY` in those calendars; localized presentation formats only after resolution; date-only presentation wins when birthtime is also present; datetime instant localization; both-created-fields-NULL fallback labeling; AX label composition; rename-field shows filename for titled note.
