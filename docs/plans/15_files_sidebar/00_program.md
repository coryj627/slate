...
| 11 | Multi-select semantics | Preserve the shipped pointer selection model and anchor, then add ⇧↑/⇧↓, ⌘A select-visible-level, and multi-item drag. A batch is **one logical core request**, not K independent Swift operations: complete preflight, progress journal, rollback attempt on failure, one consolidated result, one refresh/announcement, and one undo group. Filesystem mutations cannot be crash-atomic, so rollback failure must be reported honestly. Mixed file+folder selections allow move/delete only. |
