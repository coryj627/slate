...
| 4 | Derived metadata | One scanner-owned pipeline: display name (frontmatter `title`), effective created timestamp (frontmatter override > existing `files.birthtime_ms`), task counts (total/unfinished), word count, and preview text. Migration 031 adds only `file_meta(file_id, word_count, char_count, preview)`; it must not add a second created-time column. The scan/save paths maintain regenerable metadata in **O(changed-file)** and a census proves incremental ≡ full rescan. |
...
| 12 | Dual-pane | Default remains `tree`. FL7-1 lands the container behind an **internal gate only**; no public Settings or palette toggle appears until FL7-2 supplies the complete list pane and equivalence tests pass. When public: navigation pane (folders + tags + shortcuts) and list pane (rows of the selected container) use the standard focus-follows pattern. Both panes are complete AX trees; no datum or action is dual-pane-only. |
...
| 15 | Commands & shortcuts | `CommandRegistry`, `SlateCommandID`, and the cross-language `CommandSection` already serve the menu bar, command palette, and utility surfaces. FL appends `.sidebar` to `CommandSection` and registers every FL command through that existing system. Menu, palette, context, toolbar, keyboard, and VoiceOver rotor surfaces call the same AppState action funnels. Shortcut chords: open-shortcut `⌃1`–`⌃9` scoped to sidebar focus, collapse-all/expand-loaded, and selection back/forward; no chord is the only path. |
...
| 2 — Operations | FL-00 → FL-03 → FL-04; FL-03 → FL-05 | residual selection; one logical batch before action/import completion |
