...
**W7-7 PR 9 — #1254, contract 40 R-11 (2026-09-23).** The NVDA pass's stale "Selected: New Canvas" (F11) came from the view, not the view model: a list synchronized with its current item selected the fresh grouped view's FIRST row on every `ItemsSource` swap, and the pointer route handed that row to the view model as the user's choice. The swap now runs inside the selection-sync guard, the list sets `IsSynchronizedWithCurrentItem="False"` and its grouped rows virtualize; P7 and P10 stand unchanged. (This line first said a "synchronous-by-decision rule" stood too; no such rule can stand against locked decision 05 §4 — see #1275 below.) Current evidence: the view-hosted `TheShippedListAnnouncesOnlyTheViewModelsSelection`, `TheGuardAloneKeepsASynchronizedSwapSilent` (the guard alone, with synchronization switched back on) and `AQueryThatRemovesTheSelectionAnnouncesItsReplacementThenTheCount` (a removed selection's replacement is announced exactly once, before the count), with `GroupedRowsVirtualize` and `RecomputeScalesLinearlyInRowCount`. The announcement-stream leg of `Palette_TypingAnnouncesOnlyTheFinalCountAndSelection` is pending PR 1's desktop listener (#1244); until it lands that journey asserts the on-screen selection only.

**#1275 — the palette ranks off the UI thread (2026-09-26).** Locked decision 05 §4, principle 2 ("Callers dispatch off the UI thread") governs every FFI call the palette makes: the open-time `ListCommands` and recents load (P4), every `palette_sections` ranking (P18) and the recents write after a successful invocation (P9 step 4, P11) now run on one serialized work lane, never on the dispatcher. Each query change is a request tagged with a generation; only the latest generation's result is published, on the owning thread and atomically, so rendering, navigation and the count still read one stored result per query change (P18). The P7 selection rule and the P10 count are applied at publication, not at request, so R-11's one selection per query change holds across the hop; Enter pressed while a newer query's rows are on their way runs once they land. P9's order is unchanged — the recents transition is handed to the lane before the dismissal, and the lane's order lands it before the next open's load. Evidence: `ASlowRankLeavesTheDispatcherPumpingAndTheRowsUntouched`, `OnlyTheNewestQueryPublishes`, `TheSelectionMadeWhileARankRunsIsTheOneThePublicationKeeps`, `EnterWhileARankIsPendingRunsTheRowsItWasTypedFor`, `TheSnapshotLoadsOffTheUiThreadAndAClosedPaletteTakesNothing`, `ARecordedInvocationIsWrittenOffTheUiThread`, `TheLaneRunsWorkInHandOverOrderAndSkipsCancelledWaiters`.
