...
**Recovered read-only canvas (#1173).** If a syntactically valid JSON object has a malformed top-level section but retains at least one modeled card, keep a coherent read-only snapshot available for inspection. Outline, table, diagram, filtering, group navigation, Where-am-I and full card text remain readable. The visible, focusable status and polite announcement use core's `CanvasLoadedReadOnly` event: "Canvas opened read-only. ⟨n⟩ cards available to inspect. Repair the file and retry to edit." Use the singular form for one card. A visible, named Retry control reopens the file; only a successful editable reopen enables authoring.

Invalid JSON, a non-object root, or recovery with no modeled cards remains unavailable with the existing readable error and Retry. A valid empty canvas remains editable. Unsupported individual entries in an otherwise valid document retain the existing tolerant, editable behavior and skipped-item notice. A parse failure never supplies an editable seed or a lossy save/copy path. Fresh text inspection is explicitly read-only and closable; a previously opened editable draft is retained and cannot be committed or silently discarded when recovery replaces its document.
