...
- Duplicate: deep-copies the node (kind, size, color, unknown fields); marked-set duplicate = one entry; announced with placement. #1172 supplies `CanvasOp::CloneNode { source_id, id, x, y }`: core resolves the source in the action's current working canvas and preserves the entire stored node, changing only identity and position. Raw absent/null/empty group labels, backgrounds, file subpaths and ordered nested opaque values survive. Destination coordinates must be finite; a missing source, occupied id or invalid position refuses the whole action. The operation copies no edges or descendants implicitly; each host expands a picked group through core containment, requires complete membership and geometry, and submits one clone per member in one action. Undo removes all copies; redo restores their exact stored payloads independently of subsequent source changes.
...
As issued (filter field scoped to the open canvas narrowing outline + table by title/type/target/group; `SearchState`/`CommandPaletteModel` patterns) plus contract work: input debounced through #518 coalescing; result announcements "3 cards match" and per-row "n of m, filtered" values (t0 §3 — filter state inspectable); ⌘F binds while canvas focused (program table); **Esc-ladder position**: after M5's open-panel pre-emption and any active mode, Esc clears the filter (announced "Filter cleared — 40 cards"), and the next Esc proceeds through the remaining rungs; Clear Filter is also a palette command. Editing an active needle to empty announces that clearance exactly once (t0 §1.6). Filtered-out cards remain in the file (filter is a view, never a mutation). Ordinary traversal remains filtered; an explicit structural destination is revealed under t0 §1.6. No-selection Where-am-I uses the current displayed projection under t0 §1.4.
**Tests:** narrowing per field; selection preserved through filter/clear; last-backspace announcement and explicit-clear deduplication; announcement + values; open-panel/ordinary Escape dispositions; ordinary navigator filtering and structural reveal; readback fallback with a hidden first document row and with zero matches. See t0 §6 for the implementation obligations introduced by #1171.
