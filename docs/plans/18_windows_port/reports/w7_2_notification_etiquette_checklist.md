...
| 4 | W7-2 B2 | Scan rate and completion | Open a disposable 2,000-file vault; inspect the Vault scan progress RangeValue and capture the notification transcript through completion. | Start and finish are forced; progress is no more frequent than every 350 ms (about three per second), with the final indexed count. | `ScanAnnouncementGateTests`, `UiProgressListenerTests` | Pending (smoke scope) | Pending | Pending |
