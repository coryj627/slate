...
| W1-RT-11 | Low / P3 | Maintainability | After security/performance remediation, `FilesSidebarViewModel.cs` had grown to 2,653 lines while `WorkspaceViewModel.cs` remains 1,612 lines, with persistence, asynchronous work, command policy, and presentation projection colocated. This raises review and race-analysis cost, although no defect follows from size alone. | Remediation is staged as behavior-neutral partial units. Filter ownership is isolated in 258 lines; tree refresh/expansion in 493; and bounded import in 340, including its cancellation, state, traversal, collision and reporting contracts. Structure censuses prevent re-collapse, and the primary sidebar file is 1,614 lines. Sidebar operation ownership is complete; workspace layout/persistence remains in separate follow-up groups within the authored-file cap. |
