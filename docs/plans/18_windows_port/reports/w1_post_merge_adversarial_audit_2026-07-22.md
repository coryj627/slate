...
| W1-RT-11 | Low / P3 | Maintainability | After security/performance remediation, `FilesSidebarViewModel.cs` had grown to 2,653 lines while `WorkspaceViewModel.cs` remains 1,612 lines, with persistence, asynchronous work, command policy, and presentation projection colocated. This raises review and race-analysis cost, although no defect follows from size alone. | Remediation is staged as behavior-neutral partial units. The first extraction gives the filter pipeline one 258-line owner for its UI context, cancellation, completion, generation, query projection and date-window conversion, reducing the primary file to 2,412 lines; a structure contract prevents re-collapse. Tree/import ownership and workspace layout/persistence remain separate follow-up groups, each within the authored-file cap. |
