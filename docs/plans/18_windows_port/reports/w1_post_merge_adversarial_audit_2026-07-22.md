...
| W1-RT-11 | Low / P3 | Maintainability | After security/performance remediation, `FilesSidebarViewModel.cs` had grown to 2,653 lines while `WorkspaceViewModel.cs` remained 1,612 lines, with persistence, asynchronous work, command policy, and presentation projection colocated. This raises review and race-analysis cost, although no defect follows from size alone. | Sidebar filter/tree/import ownership is isolated in 258/493/340-line partials, reducing the primary sidebar file to 1,614 lines. Workspace restore, mutation batching and snapshot serialization now have a 212-line owner, reducing the primary workspace file to 1,415 lines. Structure censuses prevent re-collapse. Workspace layout policy remains the final behavior-neutral follow-up within the authored-file cap. |
