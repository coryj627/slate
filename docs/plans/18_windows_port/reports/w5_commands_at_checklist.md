...
| 8 | W7-3 navigation map | Sidebar action catalogs | Select a disposable file or folder, expand File actions, then Tab through its buttons. Enter a new name in File or folder name and invoke Rename; select the target again and invoke Duplicate. | File actions expose named native buttons and current availability; Rename and Duplicate operate on the selected file or folder. | `FileManagementTests`, `CommandDriftTests` | Pending | Pending | Pending |
