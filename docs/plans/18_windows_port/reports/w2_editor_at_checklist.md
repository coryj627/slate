...
W7-7 remediation of finding F8 ([#1251](https://github.com/coryj627/slate/issues/1251); not a human cell, no cell above changed): row 4's citation and embed popovers now announce their outcome (the embed preview when its result lands), and the preview text keeps a caret so Down reads its next line. The automated twins above pin the posted events, the popover name and the caret flags; the shell journey reaches the embedded content by Shift+Tab and reads a second line. An NVDA retest of row 4 is still owed.
