...
| 4 | W2-3 | Semantic activation and preview | At a link, tag, citation and embed use Ctrl+Enter; use Ctrl+E for preview and Escape to return. | The correct target or canonical unresolved result is announced; preview content is readable and focus returns. | `W2EditorInteractionTests`; `ActivatingACitationAnnouncesCoresSentenceAndNamesThePopoverWithIt`; `AResolvedEmbedAnnouncesItsPreviewWhenTheResultLands`; `AnUnresolvedEmbedIsWordedByCoreOnEverySurface`; `PreviewTextKeepsACaretAndTheScrollHostIsNotAFocusStop`; `FluentShell_UiaPatternsKeyboardFocusAndAxe_AreClean` (the hosted facts are the current evidence; the journey's announcement legs are pending PR 1's listener, #1244) | Pending | Pending | Pending |
...
| 9 | W7-1 | Braille semantic fields | Use NVDA Braille Viewer, or the reader’s braille output, on a heading and link. | Heading/link markers track the current range; record the output and braille configuration. | `EveryCanonicalKindHasItsContractAttributeThroughTheNativePeer` | Pending (owner-deferred 2026-09-18) | Pending (owner-deferred 2026-09-18) | Pending (owner-deferred 2026-09-18) |
...
W7-7 remediation of finding F8 ([#1251](https://github.com/coryj627/slate/issues/1251); not a human cell, no cell above changed): row 4's citation and embed popovers now announce their outcome (the embed preview when its result lands), and the preview text keeps a caret so Down reads its next line. The hosted twins above are the current evidence for the posted events, the popover name and the caret flags. The shell journey reaches the embedded content by Shift+Tab and reads a second line; its announcement legs (the embed result, the citation popover) are pending PR 1's desktop listener (#1244), so it claims no announcement-stream evidence yet. An NVDA retest of row 4 is still owed.
