...
8. **Fluent theme adoption (program decision 2 addendum, owner call 2026-07-19).** The shell ships on the first-party WPF Fluent theme:
   ...
   - **Contrast themes:** verify the automatic `Fluent.HC.xaml` switch against the decision-11 acceptance (all four built-in Contrast themes + one customized, runtime switching both directions, selected/disabled states, visible boundaries, no color-only meaning). Fluent HC is the mechanism; the acceptance is unchanged.
...
2. Parity behaviors: recents-first empty state, result count announcements (canonical events — the W0.5-3 `a11y_render` vocabulary carries the filter-count strings; render core events, never invent text), Enter/modifier-Enter open targets, Esc restore.
