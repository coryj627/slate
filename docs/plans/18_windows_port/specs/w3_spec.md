...
4. **Structural navigation (amended 2026-07-26 — gap_analysis G21; supersedes "works natively").** Measured with NVDA 2026.1.1 and verified in NVDA source: browse mode — and with it single-letter quick-nav — is granted by the screen reader's own per-app class selection (`treeInterceptorClass`; the generic UIA base class contains zero browse-mode wiring) and **cannot be requested through UIA** by any control in any framework (WPF, WinUI3, or otherwise). Native document apps that have it — Kindle, Acrobat, Word — have it because NVDA ships code for them. The requirement is therefore delivered in layers, with the letters always owned by the AT layer, never the app:
   - **App layer (this PR): chorded structural-navigation commands** — next/previous heading, heading level 1–6, next/previous link, list, table, embedded object — that move the real caret and announce the landing target through the canonical vocabulary (`RaiseNotificationEvent`). Modified chords pass through every mode of every AT, so they work identically under NVDA, JAWS and Narrator, today and after any AT update. Chord choices are vetted against the JAWS/NVDA binding tables (the G18 precedent — JAWS owns `Ctrl+Alt+arrows` for table reading); registered in `chords.json` with delivery evidence like every W1 command.
   - **App layer: the identity contract, test-enforced** — stable exe name, `Slate.MainWindow` and `ReadingSurface` AutomationIds asserted by FlaUI — because the AT-idiom layers key on it. Recorded in `w_c_matrix.md` as externally-dependable API. (The WPF HWND class name is per-process randomized and is never a key.)
   - **AT-idiom layers (W-E7, enhancement-only, separate from this wave):** an NVDA add-on granting genuine browse mode over the reading document (NVDA's own `h`/`k`/`l`, Elements List, user remapping; Add-on Store first, upstreaming into NVDA core à la Kindle as the endgame), and JAWS scripts mapping JAWS-convention letters onto the app's chorded commands under the user's own JAWS settings. Gated on the `RangeFromChild`-over-custom-peers spike. Every §W-C/decision-11 gate runs against **stock** ATs — the add-on and scripts may never be load-bearing.
   - **In-app single-letter navigation is an opt-in setting, default OFF.** The default must never claim keys an AT layer might; letters belong to the AT.

**Container decided (2026-07-26, amended after the caret finding): a read-only `RichTextBox` hosting the core-built `FlowDocument`** — not `FlowDocumentScrollViewer`, which has no keyboard caret (arrows re-announce one line forever; measured). The RichTextBox host keeps everything the decision rested on (same document, Text pattern, 363-char reading order) and adds a real caret, native table semantics ("table with 2 rows and 2 columns" / "out of table"), and native `Hyperlink` peers — needing *less* custom peer work. Evidence in [`w3_1_container_spike.md`](w3_1_container_spike.md). The spike added these obligations:
...
8. **Keyboard caret and focus handling.** Solved by the RichTextBox host (`IsReadOnly` + `IsReadOnlyCaretVisible` + `IsDocumentEnabled`); the remaining work is focus routing between the document, its interactive children, and the chorded commands.
...
- [ ] Linear reading verified (§W-C): NVDA + JAWS say-all reads the whole note in authored order; heading text, links, list depth, table cells and task state announced
- [ ] Chorded structural navigation verified (§W-C): every command moves the caret and is announced by NVDA + JAWS with stock settings; Tab reaches every link with an announcement
- [ ] Identity contract asserted (exe name, `Slate.MainWindow`, `ReadingSurface`) and recorded in `w_c_matrix.md`
