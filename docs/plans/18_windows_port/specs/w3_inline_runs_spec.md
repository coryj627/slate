...
**Known residual — payload duplication under a long-destination link (owner call).** `ReadingInlineRun` owns its `kind`, so every run inside one Markdown link holds its own copy of the destination. A link whose destination is kilobytes AND whose label contains many run boundaries (styles, inline code, or selected tokens) therefore retains Θ(runs × destination): measured 8.0 MB from a 6 KB source, 512 MB from 48 KB — the same shape in both hosts, since this is the canonical model, not a host artifact. Authored prose does not reach it (a realistic 2 KB tracking URL with a styled label yields 8 runs, ~16 KB), so it is recorded rather than fixed: closing it means runs referencing a shared kind table instead of owning payloads, which changes the shape §10 binds W3-1 to and the `inline_runs` golden format. Flagged at implementation time so the decision is taken deliberately, not discovered in W4.
