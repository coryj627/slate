...
2. **Mutation-parity harness (new, 2026-07-19):** the shipped §W-A harness is **read-side only** — it cannot validate rewrite safety. W5-4 ships a **differential mutation harness**: drive identical mutation sequences on both platforms over a fixture vault and diff the **final artifacts** — full file tree with byte-exact contents, link-rewrite results, op-log entries, and undo state — under the failure scenarios that matter: occupied destination (typed conflict, nothing clobbered), interruption mid-sequence, cancellation, retry-after-conflict, and undo round-trips. (#911's stress test covers the provider-level rename primitive only; it does not prove rename-with-link-rewrite or delete/batch coherence.) **Cross-platform equality is not the only oracle** — a shared-core defect passes every differential compare, so each scenario also asserts against **platform-independent expected state**: committed mutation goldens (expected final tree + bytes) plus invariant checks — unrelated files byte-unchanged, destination never clobbered, byte-exact rollback after interruption/cancellation, referentially correct link rewrites (every link resolves post-rewrite), transactionally consistent op-log entries, idempotent retry, and byte-exact undo restoration. **All scenarios green is a W5-4 ship gate** — the invariants and goldens land here, not in W8-4 (which hardens coverage, not correctness). The harness skeleton also serves W1-2's tree-CRUD and W4-4's property-rewrite rows. These are **not** ordinary serializer additions to the read-side harness.

- [ ] (each) matrix rows green; keyboard-complete; §W-C/§W-D rows green
- [ ] (W5-4) every mutation-harness scenario green against both oracles — cross-platform diff **and** the committed expected-state goldens/invariants — before ship
