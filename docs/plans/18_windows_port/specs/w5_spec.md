...
1. Command-driven file management parity (new note, new folder, rename with link-rewrite, move-to-folder, delete flows incl. confirmation semantics, Duplicate) + bulk rename sheet parity — all mutations via core (creates via O's never-clobber `create_exclusive`; renames/moves via the same rewrite engine; §W-A rewrite rows). **Depends on W1-0 (#911)** — rename/move UI may not ship before the atomic no-clobber primitive lands, the same gate W1-2 carries.
