...
7. **Peer topology unchanged.** The editor peer still has no children; the decorator adds none (there are no embedded objects in the source editor). The `EventsSource` redirect is the one place the #1088 class of bug could enter; §2.5 pins it.
...
| `Wikilink`, `Link`, `Embed` | `LinkAttribute` = the span's own range; `StyleName` = "Wikilink" / "Link" / "Embed" | speaks "link" (`link`) on entering the range | **yes** |
| `Image` | `LinkAttribute` (the target is activatable, W2-3) ; `StyleName` = "Image" | "link" | yes |
| `Tag`, `Citation` | `LinkAttribute` **if** the owner confirms (§7.1 — both are activatable via Ctrl+Enter, W2-3) else `StyleName` only | "link" / silent | owner call |
...
- **Unit (xUnit, `SlateWindows.Tests`)** — `EditorSemanticTextRangeTests`: over a fixture note containing **every** `EditorSpanKind` variant (add one to the shared markdown corpus if none has all sixteen; the W2-2 §W-A `spans` artifact then pins it cross-platform for free): attribute value at a known offset per kind; Mixed across a boundary; not-supported on plain text; `FindAttribute` forward/backward for `StyleId` and `Link`, including no-match; a document-range `StyleId` query is Mixed after one FFI call; queries during a peer update and during composition answer not-supported; `TextChanged` raised once for a 20-keystroke burst inside one tick, never inside `BeginPeerUpdate…EndPeerUpdate`; the dispatcher-affinity assertion. Use `WorkPump`/an injected clock — never wall-clock sleeps.
...
- [ ] `w_c_matrix.md` "Editor document" row amended (patterns: `Text` with `StyleId`/`Link`/`StyleName` attributes and `FindAttribute`; events: selection-changed + text-changed; evidence names the new facts); `reports/w2_editor_at_checklist.md` rows added (W7-4)
