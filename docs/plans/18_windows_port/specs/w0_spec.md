...
- [x] Full binding compiles + smoke passes; §W-E censuses green under load *(binding-surface census pins the four objects + three foreign traits; handle-lifetime / callback-concurrency / cancellation / error-mapping censuses in `apps/slate-windows/tests/SlateWindows.Tests/Censuses/`, `[Trait("census", …)]`, moderate tier per-PR + `SLATE_CENSUS_FULL=1` full tier; error-mapping totality = reflection pin of all 17 `VaultException` arms + organic triggers)*
