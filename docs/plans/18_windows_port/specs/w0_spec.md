...
- [x] Full binding compiles + smoke passes; §W-E censuses green under load *(binding-surface census pins the four objects + three foreign traits; handle-lifetime / callback-concurrency / cancellation / error-mapping censuses in `apps/slate-windows/tests/SlateWindows.Tests/Censuses/`, `[Trait("census", …)]`, moderate tier per-PR + `SLATE_CENSUS_FULL=1` full tier; error-mapping totality = reflection pin of all 17 `VaultException` arms + the `census_synthesize_vault_error` FFI census fn raising every arm end-to-end with contract-fixed fields + organic triggers)*
- [x] `slate-cli` green on Windows runner *(windows.yml step; 20/20 locally)*
- [x] Logging sink wired + tested *(App startup installs `init_host_logging`; HostLogProbe child process + census assert the stderr sink surfaces the deterministic palette-recents warn)*
- [x] §W-A skeleton harness runs on the fixture corpus (mac twin included) *(new `tests/fixtures/markdown/` corpus incl. CRLF + mixed-ending fixtures; `apps/slate-windows/tools/ParityHarness` + `ParityHarnessTests.swift` both diff against committed goldens in `tests/fixtures/parity_golden/` — cross-platform byte-identity proven transitively until W8-4's direct three-job diff; `scripts/diff-parity-artifacts.py` is the standalone diff step. The W0-1 probe + throwaway workflow retired here per §Decision, censuses seeded from it.)*
