...
| D | Diagram neighbours and the "Connects to" content — the first-ten cap and the overflow count (`GraphDiagramView.swift:851–870, 1216–1223`) | `neighborhood_ids` internal | 2 | `graph_neighbors(id, filter) -> Vec<id>` + the content as a 0a event over the FULL label list, the cap core's `GRAPH_NEIGHBOR_LABEL_CAP` (0b/0a) |
...
**Consumes.** `graph_neighborhood`, 0b's `graph_connections_tree`, `graph_ghost_note_path`, `graph_constants` (the depth clamp); 0a's `GraphReRooted`, `GraphNeighborhoodSummary`, `GraphRow`, `GraphStatus`, `GraphBlocked`; the leaf host (`RightPanePanelsViewModel.cs`) and the note-creation seam.
...
**Consumes.** 0a's `GraphWhereAmI`, `GraphPreset`, `GraphFilterCount`, `GraphMode`; 0b's `graph_filter_labels`, `graph_config_read/write`; the canvas navigator's shape (`Canvas/CanvasNavigator.cs`, the Where-am-I panel and filter field in `CanvasSurfaceView.cs`, `CanvasFilterMachine.cs`, `CanvasPreferencesViewModel.cs`).
