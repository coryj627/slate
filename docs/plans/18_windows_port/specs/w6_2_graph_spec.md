...
3. **The "accessible textual representation" is two things, and only one of them is in core.** In core: the pre-rendered `audio_summary` strings — snapshot `"{n} notes, {e} links. {o} orphans, {g} unresolved targets."` (second sentence omitted at 0/0, `" Filtered."` appended off the default filter) and neighbourhood `"{label}: {in} links in, {out} links out. Showing {k} notes within {d} links."` (`p0_spec.md` §P0-3; `session.rs:9017` and `:1990–1997`; pinned by `neighborhood_summary_verbatim_and_unknown_path_errors`, `session/tests/graph.rs:907`). In Swift only: **everything else a reader hears** — the P1 row copy (`"{label}, {n} links in, {m} links out"`, ghosts `"{label}, unresolved, {n} references"`, embeds `", embed"`, the terse collapse), `"Connections: {label}"`, the preset headlines, `"{k} of {n} shown"`, the Where-am-I composition, the forces phrases, the tier-B summary — `GraphAnnouncer.swift` (239 lines) and 26 posting sites across `AppState+Graph*.swift`, `GraphTableView.swift`, `GraphDiagramView.swift`, `ConnectionsPanel.swift`; one `// W0.5-3 residue:` marker (`GraphAnnouncer.swift:98`). `corpus.json` holds zero graph-announcer entries; `a11y.rs:14–38` names the graph announcer as "the remaining named engine, and W6-2 does for it what 0a did". **Phase 0 is therefore not optional and blocks PR A**, exactly as it did for canvas.
...
9. **Mac-side gaps found while reading (file upstream at close-out; not W6-2 scope unless a §2 row moves them).** `GraphNodeKey.make` (`GraphViewState.swift:58–73`) re-folds ghost keys in Swift with an `en_US_POSIX` workaround although core owns `ghost_key` (`graph.rs:43`) — §2 row C moves it. `"{k} of {n} shown"` is composed at two Swift sites (`AppState+GraphTable.swift:225–235`, `GraphTableView.swift:340–342`) — 0a collapses them. The name filter is diacritic-insensitive on mac (`graphNameMatches`, `AppState+GraphConfig.swift:16–20`), which no other host reproduces byte for byte — §2 row E moves it to core with the divergence recorded (the canvas's row K precedent). The tier-B threshold (1,500) and the node-diameter formula live only in Swift — §2 row H.
...
|---|---|---|---|---|
| A | **The entire announcer grammar**: `GraphVerbosity`, `GraphEvent`, the row copy and its terse collapse (`GraphAnnouncer.swift:158–170`), `"Connections: {label}"` (`:172–181`), the preset headlines (`AppState+GraphTable.swift:336–354`), `"{k} of {n} shown"` (two sites), the forces phrases (`AppState+GraphConfig.swift:138–150`), the Where-am-I composition (`AppState+GraphDiagram.swift:252–267`), the filter phrase (`:359–365`), the tier-B entry and summary strings (`GraphDiagramView.swift:496–498, 628–637`), the "Connects to … and k more" custom content (`:851–870`), and the status literals (mode, zoom, fit, pinned, unpinned, settled, loading, errors) — 26 posting sites | two workspace events only (`GraphOpensSinglePane`, `ReopenedGraph`); the two `audio_summary` strings | 1 | `A11yEvent::Graph { event: GraphA11yEvent }` + `GraphVerbosity` (0a) |
| B | The Connections tree — undirected adjacency from the neighbourhood's edges, in/out split by centre incidence, self-edges dropped, Link+Embed merged with an embed-only flag, recursion with an ancestor cycle guard, per-occurrence path ids, `localizedStandardCompare` per level (`ConnectionsPanel.swift:381–490`) | `graph_neighborhood` returns flat nodes/edges | 2 | `graph_connections_tree(path, depth, filter) -> Vec<GraphConnectionRow>` (0b) |
...
| D | Diagram neighbours and the "Connects to" content — first ten + "and k more" (`GraphDiagramView.swift:851–870, 1216–1223`) | `neighborhood_ids` internal | 2 | `graph_neighbors(id, filter) -> Vec<id>` + the content as a 0a event (0b/0a) |
| E | The name-filter predicate — `.caseInsensitive, .diacriticInsensitive` (`AppState+GraphConfig.swift:16–20`), called by both projections | none | 2 | `graph_filter_labels(query) -> Vec<id>` (Unicode simple case-fold; the diacritic divergence recorded) (0b) |
| F | `.slate/graph.json` schema, defaults, clamping, unknown-key preservation, refuse-clobber and no-downgrade generation rules, group first-match-wins and ring cycling (`GraphConfig.swift:34–45, 61`, `GraphConfigStore.swift`, `GraphConfigTests`) | none — the file is mac-only today | 2 (schema, merge policy) / 3 (I/O) | `graph_config_read/write` with core's schema (0b); the writer's I/O host-designated |
| G | The table column model — order, `directionalComparator`, the `byLabel` total-order tie-break, kind labels, every cell's text (`GraphTableView.swift:527–636`); the preset headline reuses the comparator (`AppState+GraphTable.swift:346–352`) | rows from core, unformatted | 2 | `graph_table_rows(filter, sort) -> Vec<GraphTableRow>` with core-formatted cells (0b) |
...
| J | Spatial navigation scoring — cosine threshold 0.1, `dist/cosθ`, id tie-break; structural wrap; type-ahead (`GraphDiagramView.swift:1225–1272`) | none | 2 | `graph_spatial_step(positions, from, direction) -> id?` (0b); type-ahead a host list search over core's labels |
...
2. `A11yEvent::Graph { event: GraphA11yEvent }` — one nested family under one top-level variant (the uniffi 256-variant budget, `a11y.rs:1245–1252`); one variant per family, **no free-text variant**. Minimum set (names indicative; payloads are the FFI record fields): `GraphRow{verbosity, label, kind, in_links, out_links, references, embed}` (the P1 row copy with its ghost and embed forms and the terse collapse) · `GraphReRooted{label}` · `GraphSnapshotSummary{counts}` / `GraphNeighborhoodSummary{counts}` — typed over the count records core exports on the snapshot and neighbourhood, rendered by the one formatter that also fills `audio_summary` (superseded on 2026-09-03 by `35_graph_contracts.md` 0a-7/0aD-1: an earlier draft of this line said the strings are relayed as text, which would put a whole sentence in a `String` payload; the one already-canonical site, `GraphTableView.swift:361`, stays the pattern for RELAYING the grid's own events with their priority) · `GraphPreset{preset, count, top_label?, top_in_links?}` (orphans / unresolved / most linked / no notes to rank) · `GraphFilterCount{shown, total}` · `GraphForceValue{control, percent}` / `GraphLinkDistance{value}` · `GraphLayoutSettled` · `GraphPinned{pinned}` · `GraphZoom{context?, percent}` (fit as the context, the canvas's shape) · `GraphMode{table|diagram}` · `GraphWhereAmI{row…, component, zoom, filters}` (always verbose-grade, as the canvas's) · `GraphFilterPhrase{orphans_only, attachments, ghosts}` · `GraphTierSummary{count}` and `GraphTierEntered` · `GraphNeighborsContent{labels, more}` · `GraphStatus{note}` (Graph. / Connections panel. / Diagram mode. / Table mode. / Created note … / already open / loading / no connections) · `GraphBlocked{reason}` (couldn't load, couldn't create). Every P1/P2 normative string has exactly one home.
...
4. **Four-place rule** (the W0.5-3 lesson): `a11y.rs` variants + `tests/fixtures/a11y/corpus.json` (regenerated through the artifact test's own path) + the uniffi mirror (and the coverage test's nested-family inventory, `CANVAS_NESTED_ENUMS`'s twin for the graph) + the two census mirrors (`A11yCorpusCensusTests.swift`, `A11yCorpusCensus.cs`).
5. **Mac consumes:** `GraphAnnouncer.phrase`/`rowPhrase` and the prose sites are replaced by event construction + `a11yRender`; the two `"{k} of {n} shown"` sites collapse to one event; `GraphAnnouncerTests` copy assertions move to Rust (table-driven over verbosity × event); the Swift tests keep coalescing, flush, the fire-time gate and `testNoDirectAnnouncementsUnderGraph`; `pinnedResidueSites` 29 → 28.

**Behavior pinned.** Strings byte-identical to the mac test expectations; verbosity per P1 (terse collapse); Where-am-I verbose-grade; the summaries relayed at Medium; coalescing stays host, class keys pinned in core's list.
...
**Behavior pinned.** 1. One document per workspace (the singleton), attached at every open/reopen/restore site, released when the tab closes; the generation probe refreshes on vault change (the W4-7 reveal discipline). 2. Load states: loading (status), ready, empty (`"No notes to rank."`-class copy is a preset's, the empty vault's copy is core's), error (`GraphBlocked`). 3. The table: nine columns in mac's order with core's cell text; default sort by links in descending then label; sort announced by the substrate; the row's Name is the P1 row copy (`GraphRow` at the set verbosity), ItemStatus carries the kind badge. 4. Selection: the current row writes `GraphViewState.SelectedKey`; the summary region reads `audio_summary` verbatim. 5. Activation: Open lands the note in the current tab; ghost rows offer Create note through the shell's seam. 6. Persistence: the singleton restore collapse as today (`W1WorkspaceRedTeamTests`). 7. The announcement on open is `GraphStatus{Graph.}` then the summary, mac's order.
...
**Evidence / acceptance.** A JAWS/NVDA user opens the graph, hears "Graph." and the vault's summary, walks a sortable table whose rows read as P1's copy, and opens a note from it. Matrix rows: `slate.graph.openTab` ✓; the surface row stays pending until F; `w_c_matrix.md` gains "Graph table (W6-2 PR A)".
...
**Consumes.** `graph_neighborhood`, 0b's `graph_connections_tree`, `graph_ghost_note_path`, `graph_constants` (the depth clamp); 0a's `GraphReRooted`, `GraphRow`, `GraphStatus`; the leaf host (`RightPanePanelsViewModel.cs`) and the note-creation seam.
...
**Behavior pinned.** The tree's order is core's (row B); self-edges omitted, Link+Embed collapsed (the `ConnectionsPanelTests` cases as Windows facts over the same fixtures); depth clamped 1–3 by core's constant; re-root announces `Connections: {label}` then the summary; Back returns to the prior root; a ghost's Create note lands the note and re-roots; the leaf re-roots when the active note changes unless pinned by the user's re-root (mac's rule, pinned).
...
**Consumes.** 0a's `GraphWhereAmI`, `GraphPreset`, `GraphFilterCount`, `GraphFilterPhrase`, `GraphMode`; 0b's `graph_filter_labels`, `graph_config_read/write`; the canvas navigator's shape (`Canvas/CanvasNavigator.cs`, the Where-am-I panel and filter field in `CanvasSurfaceView.cs`, `CanvasFilterMachine.cs`, `CanvasPreferencesViewModel.cs`).
...
**Behavior pinned.** Presets are parameterisations of the table (p1 §90): each sets the backend filter and sort and speaks its headline then the summary; the filter count `"{k} of {n} shown"` is coalesced with the fire-time gate; Where-am-I reads the row copy, the component, the zoom (when the diagram shows) and the active filters, verbose-grade, and renders in the panel; the verbosity level persists in `.slate/graph.json` through core and is read at every announce; the Escape ladder (panel → filter → surface) as the canvas's.
...
**Consumes.** `start_graph_layout` and the session API, 0b's `graph_neighbors`, `graph_constants`, `graph_spatial_step`; 0a's `GraphZoom`, `GraphPinned`, `GraphLayoutSettled`, `GraphTierEntered`, `GraphTierSummary`, `GraphNeighborsContent`; the canvas renderer's machinery (`CanvasRendererView.cs`, `CanvasRendererPeers.cs`, `CanvasPeerTopology.cs`, `CanvasViewportState.cs`, `CanvasTextScaleService.cs`).
...
**Evidence / acceptance.** The user switches to Diagram, hears "Diagram mode.", moves node to node with arrows hearing each row's copy, zooms and fits, and on a 2,000-node vault hears the tier-B summary and is routed to Table. Matrix rows: the four zoom ids ✓; `w_c_matrix.md` "Graph diagram (W6-2 PR D)".
...
**Consumes.** 0b's `graph_config_read/write`, the group matching; 0a's `GraphForceValue`, `GraphLinkDistance`, `GraphLayoutSettled`; the session's `set_forces`; the canvas prompt/sheet machinery for any picker.
...
**Behavior pinned.** Filter equivalence: table and diagram read one predicate (core's); groups first-match-wins with the ring as the non-colour channel; sliders announce the resulting condition once settled (`"Repel force 70 percent; layout settling… settled."` — 0a's events in mac's order); the config file round-trips with unknown keys preserved and never downgrades; a superseded generation is refused.
...
| D-6 | The graph's verbosity is its own setting in the vault file (mac), not the device-local canvas preference | Follow mac: one `GraphVerbosity` in `.slate/graph.json`; the menu is a separate submenu |
