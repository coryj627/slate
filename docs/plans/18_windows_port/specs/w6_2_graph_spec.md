...
**Evidence / acceptance.** The user filters by name, hears the count, jumps to orphans; asks Where am I on the table and reads the panel (CD-Q2's alternative, taken by the owner on 2026-09-08 — CD-24; the journey asserts the readback's text and the chord's fall-through while a load is in flight). Matrix rows: the three preset ids ✓, `slate.graph.whereAmI` ✓ (the table's readback; the diagram's zoom clause is W6-2 PR D's); `w_c_matrix.md` "Graph navigator, filter and Where-am-I (W6-2 PR C)".
