...
5. **Core's command section says what the chord table must do.** `CommandSection::Graph = 11` is doc-commented "Registry + palette + menu paths only; **P1 registers zero new chords**" (`crates/slate-core/src/commands.rs:55–61`). The mac catalog (`SlateCommands.swift:1514–1618`) carries twelve ids (W6-2 PR B2 adds a thirteenth on Windows, `slate.graph.connectionsBack`, whose mac twin is the Connections panel's `⌘[` key and not a command — §PR B2, B2-4); four are palette mirrors of focus-routed chords (`zoomIn` ⌘=, `zoomOut` ⌘-, `actualSize` ⌘0, `whereAmI` ⌃⌘I — routed by `zoomRouteTarget`/`whereAmIRouteTarget` between the canvas and the graph) and one is graph-owned (`fitGraph` ⌥⌘0). Windows already routes the canvas's four at the same chords in `ChordScope.Canvas` (`chords.json:851–1004`: `Ctrl+Alt+Shift+I`, `Ctrl+=`, `Ctrl+-`, `Ctrl+0`); the graph takes the **same chords in `ChordScope.Graph`** — disjoint delivery sites, the Ctrl+F precedent W6-1 D-2 used — and `fitGraph` proposes `Ctrl+Alt+0` (§7). The seven chordless rows stay chordless.
...
apps/slate-windows/src/SlateWindows/Graph/
  ...
  GraphViewState.cs            PR A   { SelectedKey: string?, Filter, NameQuery, Groups, Mode } — the ONLY view state
                                      (+ KindOnly, the preset's kind overlay — A-1 and R-B amended by the owner on
                                      2026-09-08, W6-2 PR C CD-23, `35_graph_contracts.md` §PR C C-4)
...
- **R-B One view state.** `GraphViewState` lives on the document (P2-5): the selected node key, the backend filter, the name query, the groups and the mode (and, amended by the owner on 2026-09-08 — W6-2 PR C CD-Q1, CD-23 — the preset's kind overlay `KindOnly`: core's `kind_only`, which no other field can express, written through `ApplyQuery` alone and never persisted); the Connections leaf re-roots through it; arrows in the diagram move `SelectedKey`; explicit table row movement writes the selection. Silent focus landing is the F5 exception: it writes no shared key and posts no movement. Where-am-I may read the validated current visible seat only when the shared key is absent (#1193); when no valid native seat is available, readback may use the sole current visible result. A present hidden key retains `NoSelection`, and an ambiguous result with no current seat has no fallback. **Amended by the owner on 2026-09-06 (W6-2 PR B2, `35_graph_contracts.md` B2D-1):** the one view state is the WORKSPACE's — constructed beside the graph relay, shared by the graph document and the Connections leaf, surviving the document's retirement and dropped with the workspace (the mac's `AppState` ownership of `graphSelectedNodeKey`); the document reads and writes it while it lives; a re-root and a Back write the selected key.
...
- **R-E Commands are table rows.** Every verb is a `ChordTable` row (section `CommandSection.Graph`, `ChordScope.Graph` for the five chorded COMMAND rows — `whereAmI` in PR C and the four viewport rows in PR D — beside the one non-command Escape disposition `windows.graph.escapeLadder`, PR C; the scope's delivered map holds two chords after PR C and six after PR D, amended W6-2 PR C CD-11), resolved in `SlateCommandRegistrar.BuildResolvers()`, palette-reachable (drift test 1), menu-backed where a menu item exists (drift test 2) with the accelerator from the table (drift test 3); the four routed chords are delivered by the focused surface, as the canvas's are. **Amended for W6-2 PR B2 (2026-09-06, §PR B2 B2-4):** a sixth chorded row, `slate.graph.connectionsBack` (`Ctrl+[`), sits in its own `ChordScope.Connections` and is delivered by the Connections leaf's body, falling through when there is nothing to pop; it has no menu item, so only drift test 1 sees it, and no mac catalog twin (the mac's Back is the panel's key) — a thirteenth graph id.
...
| D | Diagram neighbours and the "Connects to" content — the first-ten cap and the overflow count (`GraphDiagramView.swift:851–870, 1216–1223`) | `neighborhood_ids` internal | 2 | the topology entry's `neighbors` (`graph_topology(query, config) -> GraphTopology` — the visible nodes with their full payload and the visible edges, ONE record per semantic epoch (the layout handle, its generation, the query, the config), cached across the settling frames, never fetched per frame) and `graph_neighbors(query, id) -> GraphNeighbors` for one node + the content as a 0a event over the FULL label list, the cap core's `GRAPH_NEIGHBOR_LABEL_CAP` (0b/0a; 0bD-12) |
...
**Goal.** §2 rows B–K become core queries over the existing `GraphIndex`/metrics/layout: the visible set and the diagram's whole topology in one record, the Connections tree in reading order with its summary, the stable node key, the name filter, the config schema and merge policy, the table rows with formatted cells under a core column model, the constants, the action set and the option tables, the spatial and structural steps, the ghost's note path. Mac consumes each and deletes its Swift copy. The surface is `35_graph_contracts.md` §PR 0b 0b-2, which supersedes the names in §2's Target column and below (0bD-12).
...
**Builds.** The FFI entry points of `35_graph_contracts.md` 0b-2, each with a §W-A scenario: `graph_visibility`, `graph_topology` (the visible nodes with their diameters, groups, label slots, neighbours and the visible edges — one record per semantic epoch), `graph_neighbors`, `graph_connections_tree` (flat pre-order rows with the neighbourhood's counts), `graph_table_rows` (nine core-formatted cells per row, core's total order), `graph_table_columns` (the ordered column model), `GraphNode.stable_key` and `graph_stable_key_for_path`, `graph_label_matches`, `graph_constants`, `graph_node_diameter`, `graph_row_actions` and `graph_row_action_title`, `graph_spatial_step`, `graph_structural_step`, `graph_ghost_note_path`, `graph_config_default` / `graph_config_decode` / `graph_config_encode` (+ the schema, defaults, clamps, the version rule, unknown-key preservation, the canonical writer), `graph_config_matching_group`, `graph_config_next_group_style`, `graph_color_tokens` and `graph_ring_styles` (the ordered option tables). The parity harness's `graph_queries` artifact over a dedicated graph vault (contracts 0b-13), run by both twins; the Windows `ParityHarnessCensus` extended with the inventory, pinned-vector, schema and no-id facts, and a reflection census over core's surface list, so a node, a section or an id cannot slip in silence.

**Behavior pinned.** Each query's output equals the Swift derivation on the mac fixtures (the deleted Swift code's schema tests become the Rust facts) EXCEPT where `35_graph_contracts.md` records a divergence (0b-D1..D9: the key's source, fold and encoding; the label, tree and Folder order; the filter's fold and trim; the Modified cell; the config file's bytes; the group-element rule; label-only groups; the version rule; the self-edge), each executable on the mac lane by a characterization test where the old code is gone; every session result carries the generation it read and a host publishes only under a current token against its held authority (contracts design A); determinism (§P-C, §P-D) holds through the binding.
...
**Hand-off.** The FFI names, cited by A (`graph_table_rows`, `graph_table_columns`, `graph_row_actions`, `stable_key`), B (`graph_connections_tree`, `graph_ghost_note_path`), C (`graph_visibility`, `graph_table_rows`, `graph_config_decode/encode`), D (`graph_topology`, `graph_constants`, `graph_spatial_step`, `graph_structural_step`), E (`graph_config_decode/encode`, `graph_config_matching_group`, `graph_config_next_group_style`, `graph_color_tokens`, `graph_ring_styles`).
...
**Consumes.** `graph_snapshot`, `graph_generation`; 0a's `GraphRow`, `GraphSnapshotSummary`, `GraphStatus`, `GraphBlocked`, `GraphMode`; 0b's `graph_table_rows`, `graph_table_columns`, `graph_row_actions` (the ordered action specs with their titles), `graph_surface_modes` (the mode switcher's items), `stable_key` (0b merged first, #1180; the contingency did not arise), and `graph_table_default_sort` — a free function this PR adds to core's surface (twenty-four names), its FFI and the binding, so the default sort is fetched on both hosts (`35_graph_contracts.md` A-5, AD-1). Windows seams: the singleton and its events (`WorkspaceViewModel.Layout.cs:178–192, 229–233, 424–428, 475`), the canvas's attach funnel as the template (the per-tab property and attach at `WorkspaceViewModel.cs:296–309` — `35_graph_contracts.md` A-1 corrects the earlier `:289–300`; the funnel itself is `AttachTabDocumentsIfNeeded`, `WorkspaceViewModel.Bases.cs:1060–1079`; the registry `WorkspaceViewModel.Canvas.cs:40–80`), `AccessibleDataGrid.Bind` (`Grids/AccessibleDataGrid.cs:429–437`) and its `Announce` swap seam (`:64`), `ExternalSortHandler` (`:663`), `PanelWorkScheduler`, `AccessibilityNotificationDispatcher`.

**Builds.** `Graph/GraphDocumentViewModel.cs`, `GraphViewState.cs`, `GraphSurfaceView.xaml(.cs)` (header with the mode switcher — Diagram disabled until PR D — and the summary region; load/empty/error states), `GraphTableView.cs` (the grid configuration: `GraphTableGrid` automation id, the columns built from `graph_table_columns` — core's headers, which are the mac titles verbatim; no column listed in C# — row header = the label column, the kind labels from core's cells, row actions Open / Open in New Tab / Show connections / Reveal in File Tree / Create note per core's action set, activation = Open), `GraphAnnouncer.cs` (relay + coalescer over the pinned classes). `ChordTable` rows: `slate.graph.openTab` (chordless, section Graph — scope None by `Reg`'s rule for a chordless row) and the `ChordScope.Graph` member declared here for PR C's chorded rows (`35_graph_contracts.md` A-12); the placeholder text retired for Graph.

**Behavior pinned.** 1. One document per workspace (the singleton), attached at every open/reopen/restore site, released when the tab closes; the generation probe refreshes on vault change (the W4-7 reveal discipline). 2. Load states: loading (status), ready, empty (the empty vault's summary is `GraphSnapshotSummary` over zero counts; a preset's empty case is PR C's), error (`GraphBlocked`). 3. The table: nine columns in mac's order with core's cell text; default sort by links in descending then label; sort announced by the surface on adoption — the substrate neither reorders nor announces an external sort (`35_graph_contracts.md` A-5, AD-9); the row's Name is the P1 row copy (`GraphRow` at the set verbosity), ItemStatus carries the kind badge (a substrate seam PR A adds — `35_graph_contracts.md` A-6, AD-2 — Windows-only, A-D1). 4. Selection: the current row writes `GraphViewState.SelectedKey`; the summary region reads `audio_summary` verbatim. 5. Activation: Open lands the note in the current tab; ghost rows offer Create note through the shell's seam. 6. Persistence: the singleton restore collapse as today (`W1WorkspaceRedTeamTests`). 7. The announcement on an explicit open is `GraphStatus{Opened}` then `GraphSnapshotSummary`, mac's order; an ordinary activation or a restore speaks the summary alone; a reopen speaks `GraphStatus{Opened}`, then the shell's `ReopenedGraph`, then the summary when a load runs (none when the tab was already shown and loaded) — the cause policy of `35_graph_contracts.md` rule L, Term 6.

**Tests.** `GraphDocumentTests` (registry, load states, generation refresh, selection, activation), `GraphTableTests` (columns, sort determinism through the substrate, row actions, ghost availability), the announcement-seam census over `Graph/`, `GraphTabRoutingTests`' Windows twins added to `W1WorkspaceRedTeamTests`; FlaUI `GraphSurfaces_TableSortSelectionAndActivation_AreClean` (open the graph tab, walk the grid by UIA, assert row names verbatim, sort, activate, axe `graph-table`). **§W-A:** the table rows and the summary over the shared corpus byte-identical (0b's `graph_queries`). **§K:** `GraphOpenBenchmarks` — snapshot marshalling on the 1k/10k fixtures against the Windows host budgets the contracts set (500 ms median open-to-publication at 10k, 100 ms at 1k — `35_graph_contracts.md` AD-7; P's locked decision 10 budgets the layout and the backend, not the host's marshalling).
...
**Goal.** The right pane's `connections` leaf (already in the catalogue, `WorkspaceViewModel.cs:1716` — the `:1682` cited earlier has drifted) becomes the local graph. SPLIT by the owner on 2026-09-05 into two slices (§PR B in `35_graph_contracts.md`, BD-1): **B1** — core's connections tree at depth 1–3 for the note in view, the neighbourhood summary on load and depth change, the depth-one snippets, the generation refresh, ghost rows that create the note, as a standalone surface with its own document and selection over the workspace's ONE graph relay (A-1 and A-10 as amended by the owner on 2026-09-05, `35_graph_contracts.md` BD-12); **B2**, a following PR before PR D — re-root through the Show connections action (NOT on activation: the shipped mac opens on activation and re-roots only on that action, `AppState+Connections.swift:279–294`) with a back stack, `GraphReRooted` then the summary, the table's and the Bases surfaces' Show connections, and the selection shared with the graph document through `GraphViewState` (spec R-B, amended by the owner on 2026-09-06 to a workspace-owned view state — §PR B2, B2D-1); A-1 and A-10 were amended by the owner on 2026-09-05 (`35_graph_contracts.md` BD-12).

**Consumes.** 0b's `graph_connections_tree` (the tree with its summary counts — one record per load), `graph_ghost_note_path`, `graph_stable_key_for_path` (the receiver's centre-key check, §PR B rule C Term 8), `graph_constants` (the depth bounds) and B1's own `connections_filter` and `clamp_connections_depth` (§PR B, B-15); 0a's `GraphReRooted` (B2), `GraphNeighborhoodSummary`, `GraphRow`, `GraphStatus`, `GraphBlocked`; the workspace's leaf seam — the `ActiveLeaf` setter, the restore's direct write and `SyncPanels()` (`WorkspaceViewModel.cs`, not `RightPanePanelsViewModel.cs`, which is the note-panels scheduler and hosts no leaf — corrected by §PR B, IGE-9); PR A's `PanelWorkScheduler` primitive, `GraphAnnouncer` and the note-creation funnel.

**Builds.** B1: `Graph/ConnectionsLeafViewModel.cs` (the leaf's OWN `PanelWorkScheduler` document and its own selection, over the workspace's ONE `GraphAnnouncer` injected at construction — §PR B rule C Term 1, BD-2, BD-12; A-1 and A-10 as amended), the relay's move to the workspace (`NewGraphRelay` under `Graph/WorkspaceViewModel.Graph.cs`, the graph document's retirement dropping its pending classes, the stored fire-time gate on the filter class), the workspace's public `Connections` property and its `WorkspaceViewModel.Connections.cs` partial (the reveal transition, the root follow, the probe arm, the shutdown drain), `Graph/ConnectionsLeafView.cs` (a tree peer: the in/out groups as headers, rows named by `GraphRow`, embed-only and unresolved badges as ItemStatus, depth control "Local graph depth" with mac's hint, the loading/no-connections/error copy verbatim), the source-addressed generalisation of `WorkspaceViewModel.GraphCreate.cs`, `ChordTable` rows `slate.graph.showConnections`, `connectionsDeeper`, `connectionsShallower` (chordless), the leaf's registration and focus route. B2: the re-root seam and the shared selection.

**Behavior pinned.** The tree's order is core's (row B); self-edges omitted, Link+Embed collapsed (moved to Rust by PR 0b, so the Windows facts run over `graph_queries.json`'s `connections` section rather than over `ConnectionsPanelTests`' in-file builders — corrected by §PR B, BD-7); depth clamped 1–3 by core's constants; re-root announces `GraphReRooted` then `GraphNeighborhoodSummary`; Back returns to the prior root; a ghost's Create note lands the note and takes a SILENT generation refresh — it does NOT re-root (the shipped mac, `AppState+Connections.swift:255–310`; corrected by §PR B, BD-7); row ACTIVATION opens the note and does not re-root, only the Show connections action re-roots (same correction); the leaf follows the active note until the user's re-root pins it (mac's rule, pinned).
...
**Goal.** The command layer: Where-am-I (panel + announcement), the three presets, the name filter field and its count, the verbosity menu (three `CheckMenuItem`s) and the config load/save through core. The mode switcher stays A-11's (the Diagram item disabled; PR D wires the switch and its event) — no mode command exists on either host, and the spec's earlier "the mode switcher's commands" is withdrawn (`35_graph_contracts.md` §PR C, CD-11).

**Consumes.** 0a's `GraphWhereAmI`, `GraphPreset`, `GraphFilterCount`, `GraphMode`; 0b's `graph_table_rows` over the one `GraphVisibilityQuery` (the count is the rows result's), `graph_visibility`, `graph_config_decode/encode`, `graph_verbosities` (the Verbosity submenu's items, tags and titles) (0bD-12); and — added by this PR, consumed by both hosts — `GraphPreset`, `graph_preset_query` and `graph_preset_outcome` (§PR C C-2; the surface rises to twenty-eight); the canvas navigator's shape (`Canvas/CanvasNavigator.cs`, the Where-am-I panel and filter field in `CanvasSurfaceView.cs`, `CanvasFilterMachine.cs`, `CanvasPreferencesViewModel.cs`).

**Builds.** `Graph/GraphNavigator.cs`, the filter field (`GraphFilterField`, its summary and Clear), the Where-am-I panel (`GraphWhereAmIPanel`), `GraphPreferencesViewModel.cs`, `ChordTable` rows `slate.graph.orphans`, `unresolved`, `mostLinked` (chordless) and `slate.graph.whereAmI` (`Ctrl+Alt+Shift+I`, `ChordScope.Graph`); the Escape row `windows.graph.escapeLadder` in the same scope (C-7); the count region and Clear beside the field (C-5); `Graph/GraphConfigStore.cs`, the host I/O over core's codec, and `Graph/GraphConfigWriter.cs`, the application-scoped writer (one per process, serialised per vault key, a generation reserved at schedule time — rule W, C-10; PR E consumes it); the top-level Graph menu with the Verbosity submenu (C-12, C-9).

**Behavior pinned.** Presets are parameterisations of the table (p1 §90): each sets the backend filter and sort and speaks its headline (`GraphPreset`), which SUPERSEDES the generic summary for that load — mac's rule (`AppState+GraphTable.swift:235–243` after PR 0b; `35_graph_contracts.md` AD-9 corrects the earlier `:195–199`), pinned; the filter count (`GraphFilterCount`) is coalesced with the fire-time gate; Where-am-I (`GraphWhereAmI`) reads the full row copy, the component, the zoom (when the diagram shows; 0a-2b's zoom is optional since the owner's amendment of 2026-09-08, W6-2 PR C CD-24) and the active filters — always the full copy, the 0a-D1 correction — and renders in the panel: the TABLE's readback in this PR (the shared key's snapshot node, no zoom clause, answered only while the lineage is quiescent), the diagram's in PR D through the same seam shape (§PR C C-8); the mac's Table-mode chord, a silent no-op today, gets the same readback on its lane; the verbosity level persists in `.slate/graph.json` through core and is read at every announce; the Escape ladder (panel → filter → surface) as the canvas's.

**Tests.** `GraphNavigatorTests` (the presets, the ladder, the chord half, the Where-am-I verb refused and admitted through the active projection's seam — the table's readback in this PR, the diagram's in PR D; CD-24), `GraphDocumentTests` (the request lineage, the count), `GraphTableTests` (the field, the count region, Clear, the focus landing, the re-label), `GraphPreferencesTests` (verbosity with the m1 re-selection fact, `CurrentConfig`, the schedule and the flush), `GraphConfigStoreTests` and `GraphConfigWriterTests` (`GraphConfigTests`' Windows twins, the application writer); FlaUI `GraphSurfaces_NavigatorFilterAndWhereAmI_AreClean` (axe `graph-navigator`) (amended, §PR C CD-11; IGO-30).

**Evidence / acceptance.** The user filters by name, hears the count, jumps to orphans; asks Where am I on the table and reads the panel (CD-Q2's alternative, taken by the owner on 2026-09-08 — CD-24; the journey asserts the readback's text and the palette's Where Am I? enabled while quiescent; the chord's fall-through while a load is in flight is pinned in-process by GraphNavigatorTests, a FlaUI journey cannot hold a load open — amended W6-2 PR C T9, TGC-9). Matrix rows: the three preset ids ✓, `slate.graph.whereAmI` ✓ (the table's readback; the diagram's zoom clause is W6-2 PR D's); `w_c_matrix.md` "Graph navigator, filter and Where-am-I (W6-2 PR C)".

**Hand-off.** The navigator's Where-am-I READBACK seam and its availability seam (`WhereAmIAvailabilityChanged`, raised on installation and on every answerability transition — §PR C C-1, C-8), its chord map's `AddChord` shape (C-11), and the focus landing of rule F (C-17): PR D adds the FOUR VIEWPORT verbs to that map and installs the diagram's readback seam beside the table's; the mode switch is A-11's control, not a chord. No zoom seam exists in C (amended, CD-11).
...
| `slate.graph.connectionsBack` | ⌘[ (a panel key, no catalog id) | Ctrl+[ | Connections | B2 | the leaf's body delivers it, falls through with nothing to pop; palette-reachable; no menu item — added by the owner's B2 amendment of R-E (§PR B2, B2-4) |
| `slate.graph.orphans` | — | — | — | C | palette and menu |
| `slate.graph.unresolved` | — | — | — | C | palette and menu |
| `slate.graph.mostLinked` | — | — | — | C | palette and menu |
| `slate.graph.whereAmI` | ⌃⌘I | **Ctrl+Alt+Shift+I** | Graph | C (the row, the chord, the panel, the table's readback), D (the diagram's readback with its zoom clause) | the canvas's chord in a disjoint scope (D-2); 0a-2b's zoom made optional by the owner on 2026-09-08 (§PR C, C-8, CD-24) |
