...
**Consumes.** 0b's `graph_config_read/write`, the group matching; 0a's `GraphForceValue` (all four controls, `LinkDistance` among them), `GraphLayoutSettled`; the session's `set_forces`; the canvas prompt/sheet machinery for any picker.
