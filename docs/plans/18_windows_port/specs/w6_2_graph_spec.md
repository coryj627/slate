...
apps/slate-windows/src/SlateWindows/Graph/
  ...
  GraphViewState.cs            PR A   { SelectedKey: string?, Filter, NameQuery, Groups, Mode } — the ONLY view state
                                      (+ KindOnly, the preset's kind overlay — PENDING the owner's amendment of A-1
...
| `slate.graph.whereAmI` | ⌃⌘I | **Ctrl+Alt+Shift+I** | Graph | C (the row, the chord, the panel), D (the readback admitted) | the canvas's chord in a disjoint scope (D-2); the diagram's readback under 0a-2b as frozen — pending the owner's CD-Q2 (§PR C, C-8) |
