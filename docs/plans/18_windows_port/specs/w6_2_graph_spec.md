...
**Consumes.** `GraphAnnouncer.swift` (`GraphEvent` cases, `rowPhrase` `:158–170`, `phrase` `:172–181`), the 26 posting sites (§0 item 3), `GraphAnnouncerTests.swift` (the copy matrix — becomes the Rust golden), `GraphCommandsTests.testPresetAnnouncementStringsVerbatim` (`:76`), `GraphConfigTests.testForcesChangePhraseNamesOneControl` (`:97`).
