...
4. Authoring parity: full T verb set (create/delete/color/group/connect/edit/duplicate/convert/locate…), card editor, pickers, color accessibility set (APCA-verified fills, Increase Contrast collapse, nearest-preset naming) via the shared token set — seeded in W1-1 already meeting Lc ≥ 75; the fills' APCA verification is part of **this issue's** acceptance (ad hoc check, mac test approach), with W8-2 later locking the whole set behind the CI gate.
