...
- [x] 1. Consumes P's canonical model and its **accessible textual representation** (the entry-criterion artifact) + metrics substrate; accessible-first order per P's locked decisions (one model, two projections) — **PR 0b, A, B, D**. — *evidence 2026-09-14 (0b #1180, A #1181, B1 #1184, B2 #1185, D #1214; the table projection first, the diagram over the same model)*
- [x] 2. The P interaction model is the behavioral spec once shipped; same §W-A/§W-C/§W-D discipline; determinism guarantees (P rejected nondeterministic layout) hold identically through the binding — **PR A–E; the §W-A position golden in D; recorded F**. — *evidence 2026-09-15 (A–E #1181–#1215; the position golden `graph_queries.json` `layout` in D #1214, bit-identical on both twins; the end-to-end suite of F re-runs the golden through the shell — `35_graph_contracts.md` §F F1, F3)*
- [x] 3. **Phase 0 first (from W6-1):** the graph announcer is the other named residue engine (`a11y.rs:14–38`); run the same §W-G audit (announcer → core, Swift-derived structural rules → core, mac consumes) before the first Windows PR, and write its detailed spec in the W6-1 form — **PR 0a/0b; the spec above**. — *evidence 2026-09-03 (0a #1179, the announcer vocabulary in core with the mac host consuming; 0b #1180, the structural queries in core with the Swift copies deleted)*

- [x] (each) matrix rows green — *evidence 2026-09-15 (F: the thirteen `slate.graph.*` rows, the `connections` leaf row and the surface row implemented in `parity_matrix.md`; `35_graph_contracts.md` §F F7)*
- [x] (each) canonical-consumption audit (§W-G) recorded — *evidence 2026-09-15 (the register seeded in 0b #1180, closed by F's re-grep; `35_graph_contracts.md` §F F6)*
- [ ] (each) AT checklists executed, or listed as the release residual with the owner's sign-off — `reports/w6_2_graph_at_checklist.md` (F8), every human cell Pending; the issue closes on the owner's sign-off, not on PR F's merge (§F FD-6)
