...
A sheet/panel reachable from: BasesView toolbar ("Edit view filters"), the command palette ("Bases: New query"), and the Queries sidebar section (N4-3 — that entry point activates when N4-3 lands; N4-1 ∥ N4-3 run in parallel and neither blocks the other). One builder, three entry points; editing an existing view loads its AST.

### Normative rules

1. **Source picker** (05 §8.6 item 1): a labeled radio-list — All notes / Folder… / Tag… / Recently edited (days stepper) / Linked from note… / **Tasks** (decision 8). Folder/tag/note pickers are searchable lists fed by the shipped inventories (baseline facts) — never free-text-only. Maps to `QuerySource` (05 §8.2) / `source: tasks`. On save-as-`.base`, **source compiles into filters** (Obsidian has no source clause): Folder ⇒ `file.inFolder(…)`, Tag ⇒ `file.hasTag(…)`, Recently edited ⇒ `file.mtime >= now() - "Nd"`, **Linked-from ⇒ `link("<note>").linksTo(file.file)`** (same serialization N0-5 rule 3 pins for DQL `outgoing()` — gap G6); the builder round-trips each back by recognizing its own canonical first-conjunct shape — §N-G test.
...
3. Explicit keyboard commands (05 §8.6): Add condition / Remove condition / Edit condition — registry commands, buttons in the row, no drag anywhere. VO reads each row as "Condition N: <property> <operator> <value>" and the list announces the combinator: "Combined with AND" (vendored checkpoints grammar, ../02_milestone_brief.md).
...
1. **Queries sidebar section**: a new `Leaf` case (`queries`) listing saved queries (name, description tooltip) + `.base` files (from `bases_list`) in two labeled groups; Return opens (saved query ⇒ ephemeral tab via `open_saved_query` **with `this_path: None`** — a saved query has no backing file, so a `this`-mentioning query in a plain tab surfaces the N1-1 rule-5 error with an actionable hint "Dock to sidebar to follow the active note" (N4-4); `.base` ⇒ funnel); context/keyboard actions: run, edit in builder, rename, delete (confirm), export as `.base` (N2-2 rule 3), pin, dock to sidebar (N4-4).
