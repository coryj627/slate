...
| O12 | Dataview task-field semantics | `created`/`completion`/`start`/`fullyCompleted` ⇒ `Unsupported` (tasks table has due/scheduled/priority/recurrence only — deliberate migration-008 shape). DQL `completed` means `"x"` **lowercase only**, so it maps to `task.status == "x"`, not Slate's `task.completed` (which is `{'x','X'}`) — a `- [X]` task would otherwise silently flip | N0-5 rule 6. Conversely Slate indexes `priority`, which Dataview explicitly does not support. |
