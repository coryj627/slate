...
Pages (all under `file.`): `name, folder, path, ext, link, size, ctime, cday, mtime, mday, tags` (subtags broken down per level), `etags` (explicit only), `inlinks, outlinks, aliases, tasks, lists, frontmatter, day`, `starred`. `file.day` scans merged fields in source order for case-insensitive `date`/`day`: a scalar date wins; an array counts only when element 0 is a date; a scalar link extracts a date from path, then subpath, then display; otherwise a date embedded in the Markdown title is used. Tasks (top-level in TASK queries; inherit all page fields): `status` (raw char), `checked` (status is neither empty nor the single unchecked space), `completed` (`x` or `X` on this task), `fullyCompleted` (incl. subtasks), `text, visual, line, lineCount, path, section, tags, outlinks, link, children, task, annotated, parent, blockId`, plus shorthand-derived dates `due` 🗓, `completion` ✅, `created` ➕, `start` 🛫, `scheduled` ⏳. **Owner amendment 2026-07-10:** these two predicates follow the current executable Dataview source; the earlier lowercase-only/non-space summary was stale. **No `priority` field** — Tasks-plugin priority/recurrence shorthands are explicitly unsupported by Dataview (Slate's index *does* store priority; the delta runs the other way).

### §8.5 Function inventory (complete)

Constructors: `object, list/array, date(any), date(text, luxonFmt), dur, number, string, link(path,[display],[embed]), embed, elink, typeof`. The one- and two-argument `link` registrations vectorize; the three-argument registration is scalar-only. Numeric: `round(n,[digits]), trunc, floor, ceil, min, max, sum, product, reduce(array, "+|-|*|/|&||"), average, minby, maxby` (empty-array aggregates ⇒ null). Containers: `contains, icontains, econtains, containsword, extract, sort, reverse, length, nonnull, firstvalue, all, any, none, join(array,[delim=", "]), filter, unique, map, flat(array,[depth=1]), slice`. Strings: `regextest, regexmatch` (whole-string), `regexreplace, replace` (literal, all), `lower, upper, split` (regex delimiter; capture groups spliced), `startswith, endswith, padleft, padright, substring, truncate`. Utility: `default/ldefault, display, choice(bool,l,r), hash(seed,[text],[variant]), striptime, dateformat` (Luxon; returns a *string*), `durationformat, currencyformat, localtime, meta(link)` (`.display/.embed/.path/.subpath/.type`). Most functions vectorize over lists element-wise. Functions are legal everywhere **except FROM**.
