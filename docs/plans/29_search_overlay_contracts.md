...
- **SR-3 — Recents are not ported in this issue.** Mac keeps a per-vault
  `.slate/search-recents.json` (max 20 entries, 64 KiB,
  `SearchRecentsStore.swift:33-124`) and the empty-query state shows them.
  Windows ships the `Type to search.` empty state only. Recents are a
  separate persisted store with its own corruption and size semantics;
  folding them in would double this issue's surface.
