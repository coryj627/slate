...
**UI.** The autocomplete settings live under the **Editor Intelligence** grouping in `SettingsView` (program locked decision 11), **shared with Milestone X — LaTeX authoring aids** ([X3-1 #586](https://github.com/coryj627/slate/issues/586), `docs/plans/12_latex_suite/`) — one settings home for opt-in editor-enhancement features, not two competing top-level tabs. Whichever of V2-1 / X3-1 lands first *introduces* the Editor Intelligence container; the second *joins* it; native tab-interface AT shape preserved either way. Within it, an `Autocomplete` section: master enable, trigger group (autoTrigger, autoFocus, min-lengths, max-look-back), completion group (insertion mode, ignore-diacritics, insert-space/period), and a per-provider enable list (each with its sub-options: LaTeX-in-code, front-matter ignore-case/tag-suffix). `Form` sections wrapped `.accessibilityElement(children: .contain)`; every control labeled + hinted.
