...
- **Oracle census `census_barnes_hut_matches_exact`:** random graphs ≤ 500 nodes, forced-BH vs forced-exact, per-iteration force vectors within 5% relative (magnitude-weighted RMS) tolerance, checked at multiple checkpoints along a real trajectory — plus the qualitative invariants (no NaN, bounded) at 10k forced-BH. **Implementation note (P2-1 landed):** the original "final layouts within `0.05 * k` per node" clause was found to be physically unachievable and was replaced — a force-directed system has many local minima, so two INDEPENDENT trajectories differing by BH's per-step approximation settle into DIFFERENT minima (measured ~30× the layout scale apart, not `0.05*k`), regardless of BH correctness. The achievable, meaningful "final layout" oracle instead asserts that a BH-DRIVEN settled layout is near a stationary point of the EXACT energy — its exact net-force residual is far below the typical mid-solve force (`barnes_hut_driven_layout_settles_near_an_exact_minimum`) — plus the per-iteration force agreement above.
