...
- PageRank: `petgraph::algo::page_rank`, damping 0.85, **fixed 40 iterations** (0.8.3's signature is already iteration-based: `damping_factor, nb_iter`), f64, on the Link-kind subgraph (embeds excluded). **Never call it on the live `StableDiGraph`**: 0.8.3's implementation iterates `0..node_count` via `from_index(i)` and is wrong/panicky once removals leave holes (`node_bound() > node_count()`), and holes are routine (ghost removal, `apply_file_removed`). Build the compact Link-kind-only temporary `petgraph::Graph` per `MetricsSnapshot` (needed anyway to exclude embeds), in key order, and map ranks back through the node list.
