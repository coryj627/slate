...
**Status:** 📝 Specs locked (2026-07-04); **baselines re-verified against main 2026-07-11** (post-M/O/T + HIG audit — hook-site, grid, event-listener, chord-ownership, and Bases-degree facts refreshed in the specs; locked decisions unchanged). Implementation started 2026-07-11. GH [milestone 16](https://github.com/coryj627/slate/milestone/16), issues [#550–#562](https://github.com/coryj627/slate/milestone/16) (P0: #550–553 · P1: #554–556 · P2: #557–561 · docs: #562). Supersedes the Phase-1/Phase-2 ordering sketched in [`../01_detailed_roadmap.md`](../01_detailed_roadmap.md) §7 and the P row of [`../06_v1_milestones.md`](../06_v1_milestones.md) (owner decision 2026-07-04: accessible model ships first; the visual graph is a projection of it). Evidence base: [`01_research_brief.md`](01_research_brief.md).
...
| 2 — Accessible navigator | P1-1, P1-2, P1-3; P2-1 runs in parallel (backend-only) | Wave 1 complete. P1-2 builds on `AccessibleDataGrid` v2 (**landed**, #519 closed 2026-07-04; consumers to copy: BaseContainerView, CanvasTableView). |
| 3 — Visual projection | P2-2 → P2-3 → (P2-4 ∥ P2-5) | P2-1 + Wave 2 (P2-5's selection sync targets P1 surfaces) |
| 4 — Close-out | P-D | Wave 3 |

**Sequencing vs. other programs:** U1 shell (landed) provides the tab seam; U4's right-pane leaf registry (landed surfaces in `RightPaneView.swift`) hosts the leaf. The Canvas program (T) is **code-complete** (25/25, 2026-07-05) and `AccessibleDataGrid` v2 (#519) landed — the former Wave-2 gate is satisfied; every T pattern P adopts is now landed code. New sequencing concern (2026-07-11): HIG-audit chord work #848/#863 (branch `claude/chords-and-zoom`) touches the zoom-router seam P2-3 joins — see p2_spec baseline.

## Relationship to other milestones (do not duplicate)

- **N — Bases (updated 2026-07-11):** N shipped `file.inDegree`/`file.outDegree` early with its own SQL-side computation — **embed-inclusive**, external-exclusive, reference-distinct links-table counts (test-pinned in `bases_engine.rs`), i.e. N's `inDegree` ≡ P0-2's `in_links + in_embeds`. This is a **deliberate delta, not a bug**: the graph splits Link/Embed (the orphan rule needs the split); Bases folds them; P1-2's table shows both columns so nothing is hidden; `docs/help/bases.md` gets one clarifying line in P-D. Do not "unify" the numbers silently in P0-2. `file.isOrphan`/`file.cluster`/`file.shortestPathTo` remain reserved (N-E5) and get their substrate from P0-2's `MetricsSnapshot` — those N-E5 fields must adopt P0-2's semantics (orphan = zero Link-kind degree, embeds don't rescue; deterministic component labels), and a Bases adoption of MetricsSnapshot totals for degrees is N-E5 follow-up material.
- **T — Canvas:** P reuses T's normative patterns wholesale: announcer pattern + verbosity (t0 interaction contract; P builds a `GraphAnnouncer` on the `CanvasAnnouncer` shape — see p1_spec baseline), per-element AX on a visual surface (#367, landed as `CanvasCardAXElement`), `AccessibleDataGrid` v2 (#519), viewport zoom chords ⌘= / ⌘- / ⌘0 via a **single focus-routed menu owner** (see p2_spec baseline; supersedes "scoped to surface focus" phrasing), "Where am I?" readback (⌃⌘I, action-side graph-scope guard). Where T defined a convention, P adopts it rather than inventing a sibling.
